...
# Version: 0.4.0    Date: 2025-10-20 (Europe/Zurich)
# Spezifikation: Übergangsjahrgänge (Frauen 1961–1969), Buttons 63/64/65 (65 grau),
# monatlicher Start, Planungsdauer 65–100 (Default 85), Blau/Orange-Linien,
# Break-even als Zahl, Urteilskarten, (i)-Popup mit Ja/Nein + ★-Rating.
# Start: pip install streamlit altair pandas ; streamlit run finaura_ahv_vorbezug_app_v0_4_0.py
...
<style>
  ...
  .legend { display:flex; gap:18px; align-items:center; margin:8px 0 8px 0;}
...
with st.sidebar:
    ...
    with st.expander("Annahmen"):
        st.caption("Kumulierte Renten ohne Diskont. Kürzung 6.8 % p.a. anteilig pro Monat, max. 20 %.")
        st.caption("Inflation 1.2 % (nur Info). Zuschläge einkommens-/jahrgangsabhängig → exakt in **FINAURA Pro** (IK-Upload).")
...
with left:
    ...
    bF, bM = st.columns(2)
    if bF.button("Frau", use_container_width=True): sex = "Frau"
    if bM.button("Mann", use_container_width=True): sex = "Mann"
...
""")
        ...
        if c63.button("63", use_container_width=True): choice = 63
        if c64.button("64", use_container_width=True): choice = 64
...
# ---------- Chart & Output ----------
with right:
    # Legende oben (wie im Mock)
    if early_enabled:
        if months_early >= 12:
            lbl = f"{months_early//12} Jahr(e)"
        elif months_early > 0:
            lbl = f"{months_early} Monate"
        else:
            lbl = "0 Monate"
    else:
        lbl = "—"
...
<div class='legend'>
  ...
  <span><span class='dot dot-orange'></span> Vorbezug {lbl}</span>
</div>
""", unsafe_allow_html=True)
