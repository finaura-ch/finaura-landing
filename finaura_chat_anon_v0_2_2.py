...
with role[0]:
    ...
    if "customer_id" in st.session_state:
        ...
        if threads:
            options = [f"#{t['id']} – mit {t['handle']} – {t['status']}" for t in threads]
            # --- Ticket-Selectbox (Variante B: #id – anon_id – status) ---
def _fmt_ticket_option(t: dict) -> str:
    anon = t.get('anon_id') or '—'
    status = t.get('status') or ''
    return f"#{t.get('id')} – {anon} – {status}"

options = [_fmt_ticket_option(t) for t in tickets]
choice = st.selectbox("Ticket wählen", options=options)


selection = choice  # Backwards-compat: keep 'selection' variable
def _extract_ticket_id(option_str: str):
    try:
        if option_str.startswith('#'):
            num = option_str.split(' ', 1)[0][1:]
            return int(num)
    except Exception:
        pass
    return None

selected_ticket_id = _extract_ticket_id(choice)
selected_ticket = next((t for t in tickets if t.get('id') == selected_ticket_id), None)
# --- Ende Selectbox ---
sel_id = int(selection.split("–")[0].strip("# ").strip())
...
with role[1]:
    ...
    if "planner_id" in st.session_state:
        ...
        if threads:
            options = [f"#{t['id']} – mit {t['anon_id']} – {t['status']}\" for t in threads]
            selection = st.selectbox("Wähle eine Unterhaltung", options, key="planner_threads")
            sel_id = int(selection.split(\"–\")[0].strip(\"# \").strip())
            st.session_state["p_thread_id"] = sel_id
        else:
