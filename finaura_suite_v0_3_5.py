...
Run:
    streamlit run finaura_suite_v0_3_5.py
"""
__version__ = "0.3.5"
...
# Altair optional (Chart fällt sonst einfach weg)
try:
...
with tabs[1]:
    ...
    with colL:
        ...
        fixed = st.number_input("Fixkosten pro Monat (CHF)", min_value=0, step=50, value=3200,
                                help="Miete, Krankenkasse, ÖV, Versicherungen, Abos …")
        living = st.number_input("Lebenshaltung/Variabel (CHF)", min_value=0, step=50, value=2500,
                                 help="Lebensmittel, Freizeit, Sonstiges")
        reserve = st.number_input("Liquiditäts-Reserve (CHF)", min_value=0, step=100, value=6000,
                                  help="Cash / Tagesgeld – für Notgroschen")
    ...
    with colR:
        ...
        # Chart (nur wenn Altair vorhanden)
        if HAS_ALTAIR:
            try:
...
with tabs[2]:
    ...
    if st.button("➡️  Chat-Modul öffnen (v0.2.7)"):
        st.success("Das Chat-Modul läuft separat. Öffne es über deinen Shortcut/Launcher. (Details in der internen Doku.)")
