# FINAURA — AHV Vorbezug (Free) — v1.1
# Fix: Break-even korrekt (Normal >= Vorbezug), konsistente Urteile & Linie
...
def berechne_kurven(start_alter_monate:int, planungsalter_jahre:int, jahresrente:float):
    """Erzeugt kumulierte Kurven & Break-even: erstes Alter mit (Normal >= Vorbezug)."""
    ...
    # Break-even: erstes Alter, an dem kum_Norm >= kum_Vor (Normal zieht gleich/ vorbei)
...
def age_minus_one_month(age:float):
    """Alter minus 1 Monat (für 'Bis Alter …')."""
...
""", unsafe_allow_html=True)
    ...
    if early_enabled:
        ...
        if be_age is not None:
            # Y am BE ~ mittlerer Wert beider Linien
            ...
            be_text = alt.Chart(be_df).mark_text(
                ...
                x="Alter:Q",
                y=alt.value(0),
                text=alt.value(f"Break-even • {format_age(be_age)}")
            )
        ...
        st.markdown("---")
        # Urteile gemäß Definition: bis Monat vor BE → Vorbezug; ab BE → Normal
        if be_age is None:
            ...
            st.markdown(f"<div class='pill good'>Bis Alter <b>{format_age(bis_age)}</b>: Vorbezug lohnt sich.</div>", unsafe_allow_html=True)
            st.markdown(f"<div class='pill warn'>Ab Alter <b>{format_age(be_age)}</b>: Der normale Bezug ist vorteilhafter.</div>", unsafe_allow_html=True)
...
st.caption("© FINAURA · Free Flagship · v1.1 · Break-even fix & konsistente Urteile.")
